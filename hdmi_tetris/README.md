...
## Tetris Additions and Optimizations

We integrated a minimal Tetris game and a per-pixel renderer that drives the HDMI pipeline:

- `tetris_game.v`: Holds a 10×20 board and current piece state. Provides a combinational read port `(q_x,q_y)->q_color` so the renderer can fetch board colors without writing to the memory.
- `tetris_video.v`: Computes board cell coordinates from the active pixel `(x,y)`, overlays the falling piece, and outputs RGB. Division/modulo by 24 for tile math were replaced with simple compare/shift-add logic to keep synthesis fast.
- `top.v`: Wires the game <-> renderer query handshake and feeds the renderer’s RGB into the HDMI encoder.

### Resource overflow fix (important)

An early version attempted to clear and compress completed lines in one clock tick using large for-loops. The synthesizer unrolled this into enormous mux networks (≈155k LUT equivalent), exceeding the device. We fixed this by introducing a tiny state machine in `tetris_game.v` that:

- SCANs one row per tick to detect full lines
- SHIFTs one row per tick to move rows down and clear the top

This keeps hardware small and predictable. Current utilization after PnR:
...
PnR completed in ~15s on the reported machine and generated the bitstream `impl/pnr/hdmi_test.fs`.

### Notes on timing constraints

We target a conservative 25.2 MHz pixel clock (from 126 MHz / 5). An SDC is included for synthesis timing; you can optionally propagate it into PnR for richer reporting. At this pixel rate on GW1NR-9, timing is typically met comfortably.
